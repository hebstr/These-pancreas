...
::: callout-warning
# Avant-dernière version 3.0.0

- plan OK
- méthode OK
- résultats OK
- discussion > principaux résultats OK
- **reste à faire : compléter discussion > limites + détails lecture/interprétation des modèles**
:::
...
Il s'agissait d'une étude observationnelle, rétrospective, multicentrique, conduite dans quatre centres hospitaliers ou hospitalo-universitaires français : CHU Nantes, CHD La Roche-sur-Yon, CHU Angers et CHU Lyon.
...
Une analyse de sensibilité conservant l'échelle de temps depuis le diagnostic a été conduite, *les patients y entrant dans l'ensemble à risque à la date de leur chirurgie.*
...
Dans un premier temps, `r n$eligible` patients ont été retenus sur les critères d'éligibilité.
...
.descr <- lst(
  ...
  delai = map(
    ...
    ~ tbl_med_iqr(tbl_tumor, "time_diag_chir", column = .x)
  ),
  recidive = lst(
    n = sum(df$recidive_type != "Aucune"),
    pct = style_pct(mean(df$recidive_type != "Aucune")),
    locale = tbl_n_pct(tbl_tumor, "recidive_type", level = "Locale"),
    meta = tbl_n_pct(tbl_tumor, "recidive_type", level = "Métastatique"),
    mixte = tbl_n_pct(
      tbl_tumor,
      "recidive_type",
      level = "Locale et métastatique"
    )
  ),
  ...
  resec = map(
    ...
    ~ tbl_n_pct(
      tbl_tumor,
      "chir_resec",
      level = "Résécable d'emblée",
      column = .x
    )
  ),
  adapt = lst(
    induc = tbl_n_pct(tbl_ttt_induc, "induc_adapt_pct", level = ">20%"),
    adj = tbl_n_pct(tbl_ttt_adj, "adj_adapt_pct", level = ">20%")
  ),
  tox_bras = map(
    lst(adj = "stat_1", periop = "stat_2"),
    ~ tbl_n_pct(tbl_tox_adj, "adj_ei", column = .x)
  ),
  tox_induc = map(
    lst(
      vom = "induc_ei_vom",
      neuro = "induc_ei_neuro",
      asth = "induc_ei_asthenie"
    ),
    ~ tbl_n_pct(tbl_tox_induc, .x)
  ),
  tox_adj = map(
    lst(
      dig = "adj_ei_dig",
      neuro = "adj_ei_neuro",
      vom = "adj_ei_vom",
      asth = "adj_ei_asthenie",
      neutrop = "adj_ei_neutrop"
    ),
    ~ tbl_n_pct(tbl_tox_adj, .x)
  )
)
```
...
L'âge médian au diagnostic était de `r .descr$age$med` ans (IQR `r .descr$age$iqr`), `r .descr$homme$n` patients (`r .descr$homme$pct`) étaient des hommes.
...
Le délai médian entre le diagnostic et la chirurgie était de `r .descr$delai$total$med` mois (IQR `r .descr$delai$total$iqr`), avec un écart marqué entre les deux protocoles : `r .descr$delai$adj$med` mois (IQR `r .descr$delai$adj$iqr`) dans le groupe adjuvant seul contre `r .descr$delai$periop$med` mois (IQR `r .descr$delai$periop$iqr`) dans le groupe péri-opératoire.
Au terme du suivi, `r .descr$recidive$n` patients sur `r n$inclus` (`r .descr$recidive$pct`) avaient récidivé : `r .descr$recidive$locale$n` récidives locales (`r .descr$recidive$locale$pct`), `r .descr$recidive$meta$n` métastatiques (`r .descr$recidive$meta$pct`) et `r .descr$recidive$mixte$n` à la fois locales et métastatiques (`r .descr$recidive$mixte$pct`).
...
Les `r n$chimio_periop` patients du groupe péri-opératoire ont tous reçu au moins une cure de chimiothérapie d'induction, avec une nombre médian de `r .descr$induc_nb$med` cures (IQR `r .descr$induc_nb$iqr`).
Une chimiothérapie adjuvante a été délivrée à `r .descr$adj_chimio$total$n` patients (`r .descr$adj_chimio$total$pct` des inclus) : `r .descr$adj_chimio$adj$n` sur `r n$chimio_adj` dans le groupe adjuvant seul et `r .descr$adj_chimio$periop$n` sur `r n$chimio_periop` dans le groupe péri-opératoire.
Le nombre médian de cures de chimiothérapie adjuvante était de `r .descr$adj_nb$adj$med` (IQR `r .descr$adj_nb$adj$iqr`) dans le groupe adjuvant seul contre `r .descr$adj_nb$periop$med` (IQR `r .descr$adj_nb$periop$iqr`) dans le groupe péri-opératoire (la phase d'induction ayant déjà délivré une partie du traitement).
Toutes phases confondues, le nombre total médian de cures était de `r .descr$n_cures$med` dans les deux groupes.
...
Les nausées et vomissements étaient l'évènement indésirable le plus fréquent, chez `r .descr$tox_induc$vom$n` patients (`r .descr$tox_induc$vom$pct`), devant la neurotoxicité (`r .descr$tox_induc$neuro$pct`) et l'asthénie (`r .descr$tox_induc$asth$pct`).
...
La diarrhée et la neurotoxicité étaient les évènements les plus fréquents, chez `r .descr$tox_adj$dig$n` patients chacune (`r .descr$tox_adj$dig$pct`), devant les nausées et vomissements (`r .descr$tox_adj$vom$pct`), puis l'asthénie et la neutropénie (`r .descr$tox_adj$asth$pct`).
...
{{< script scripts/tbl_model_pois.R >}}
...
df |>
  ...
  arrange(centre) |>
  mutate(date_lastnews = format(date_lastnews, "%d/%m/%Y")) |>
  gt::gt() |>
  gt::cols_label(
    centre = "Centre",
    date_lastnews = "Date de point"
  ) |>
...
- étude rétrospective observationnelle, recueil sur données du dossier patient informatisé

- pas de randomisation : différences entre groupes ne s'interprètent pas comme effet propre du groupe

- biais de temps immortel favorable au groupe péri-opératoire

- restriction aux patients opérés impose la chirurgie comme origine de la survie globale (délais diagnostic-chirugie par construction différents entre les groupes)

- exclusion des décès précoces post-diag et post-op : allonge les estimations de survie

- censure administrative : surestime le suivi réel

- nombre d'évènements insuffisant / manque global de puissance : impact sur variables à inclure dans les analyses multivariées et interprétation des coefficients
...
#| tbl-cap: "Facteurs d'inflation de la variance (@tbl-model-pois)"
...
#| tbl-cap: "Test des risques proportionnels de la survie globale (@tbl-model-coxph-os)"
...
#| tbl-cap: "Test des risques proportionnels de la survie sans récidive (@tbl-model-coxph-pfs)"
